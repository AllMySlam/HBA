...
void PC2Entropy(double& Entropy, int part_num)
{
    ...
    for (size_t i = 0; i < pc->points.size(); i++) {
        ...
        int                numberOfNeighbors =
            kdtree.radiusSearch(pc->points[i], 0.3, pointIdxRadiusSearch, pointRadiusSquaredDistance);
...
int main(int argc, char** argv)
{
    ...
    double accum = 0.0;
    std::for_each(std::begin(Entropys), std::end(Entropys), [&](const double d) { accum += (d - mean) * (d - mean); });
