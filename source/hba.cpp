    ...
    for (int i = thread_id * part_length; i < (thread_id + 1) * part_length; i++) {
        ...
        for (int loop = 0; loop < layer.max_iter; loop++) {
            ...
            for (size_t j = 0; j < WIN_SIZE; j++) {
                ...
                cut_voxel(surf_map, *src_pc[j], Eigen::Quaterniond(x_buf[j].R), x_buf[j].p, j, layer.voxel_size,
                          WIN_SIZE, layer.eigen_ratio);
            }
...
void parallel_tail(LAYER& layer, int thread_id, LAYER& next_layer)
{
    ...
    for (uint i = thread_id * part_length; i < thread_id * part_length + left_gap_num; i++) {
        ...
        for (int loop = 0; loop < layer.max_iter; loop++) {
            ...
            for (size_t j = 0; j < WIN_SIZE; j++) {
                ...
                t0 = ros::Time::now().toSec();
                cut_voxel(surf_map, *src_pc[j], Quaterniond(x_buf[j].R), x_buf[j].p, j, layer.voxel_size, WIN_SIZE,
                          layer.eigen_ratio);
    ...
    if (layer.tail > 0) {
        ...
        for (int loop = 0; loop < layer.max_iter; loop++) {
            ...
            for (size_t j = 0; j < layer.last_win_size; j++) {
                ...
                cut_voxel(surf_map, *src_pc[j], Quaterniond(x_buf[j].R), x_buf[j].p, j, layer.voxel_size,
    ...
    printf("load pcd %.2fs %.2f%% | undistort pcd %.2fs %.2f%% | "
           "downsample %.2fs %.2f%% | cut voxel %.2fs %.2f%% | recut %.2fs %.2f%% | trans %.2fs %.2f%% | solve %.2fs "
           "%.2f%% | "
           "save pcd %.2fs %.2f%%\n",
...
void global_ba(LAYER& layer)
{
    ...
    for (int loop = 0; loop < layer.max_iter; loop++) {
        ...
        for (int i = 0; i < window_size; i++) {
            ...
            dsp_t += ros::Time::now().toSec() - t0;
            t0 = ros::Time::now().toSec();
            cut_voxel(surf_map, *src_pc[i], Quaterniond(x_buf[i].R), x_buf[i].p, i, layer.voxel_size, window_size,
                      layer.eigen_ratio * 2);
            cut_t += ros::Time::now().toSec() - t0;
        }
        t0 = ros::Time::now().toSec();
