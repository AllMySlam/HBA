...
class LAYER {
  public:
    ...
    void init_parameter(int pose_size_ = 0)
    {
        ...
        printf("layer_num %d | thread_num %d | pose_size %d | max_iter %d | part_length %d | gap_num %d | "
               "last_win_size %d | "
               "left_gap_num %d | tail %d | left_size %d | left_h_size %d | j_upper %d | "
               "downsample_size %f | voxel_size %f | eigen_ratio %f | reject_ratio %f\n",
               layer_num, thread_num, pose_size, max_iter, part_length, gap_num, last_win_size, left_gap_num, tail,
               left_size, left_h_size, j_upper, downsample_size, voxel_size, eigen_ratio, reject_ratio);
    }
};
  ...
  public:
        ...
        for (int i = 0; i < layers[cur_layer_num].thread_num; i++) {
            if (i < layers[cur_layer_num].thread_num - 1) {
                for (int j = 0; j < layers[cur_layer_num].part_length; j++) {
                    ...
                    layers[cur_layer_num + 1].pose_vec[i * layers[cur_layer_num].part_length + j] =
                        layers[cur_layer_num].pose_vec[index];
                }
            }
            else {
    ...
    void pose_graph_optimization()
    {
        ...
        graph.add(gtsam::PriorFactor<gtsam::Pose3>(
            0, gtsam::Pose3(gtsam::Rot3(init_pose[0].q.toRotationMatrix()), gtsam::Point3(init_pose[0].t)),
            priorModel));

        for (uint i = 0; i < init_pose.size(); i++) {
            if (i > 0) {
                initial.insert(
                    i, gtsam::Pose3(gtsam::Rot3(init_pose[i].q.toRotationMatrix()), gtsam::Point3(init_pose[i].t)));
            }
        ...
        for (int i = 0; i < pose_size - 1; i++) {
            for (int j = i + 1; j < pose_size; j++) {
                ...
                gtsam::NonlinearFactor::shared_ptr      factor(new gtsam::BetweenFactor<gtsam::Pose3>(
                    i * pow(GAP, total_layer_num - 1), j * pow(GAP, total_layer_num - 1), gtsam::Pose3(R_sam, t_sam),
                    odometryNoise));
                graph.push_back(factor);
            }
        }
